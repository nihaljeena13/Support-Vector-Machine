...
for m in range(2, 58):
	...
	y_pred_2 = model.fit(X_train_m, y_train).predict(X_test_m)
	acc2.append(metrics.accuracy_score(y_test,y_pred_2))
...
for m in range(2,58):
	...
	y_pre_3 = model.fit(X_train_m, y_train).predict(X_test_m)
	acc3.append(metrics.accuracy_score(y_test,y_pre_3))
	print('\tm = ' + str(m) + ',\t\taccuracy = ' + str(accuracy_score(y_test, model.fit(X_train_m, y_train).predict(X_test_m))))
plt.plot(acc3)
plt.ylabel('accuracy')
plt.xlabel('m(number of features selected)')
